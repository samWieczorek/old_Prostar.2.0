...
#'
#' @noRd 
mod_select_keyID_from_MSnset_server <- function(id, dataIn){
  ...
  moduleServer(id, function(input, output, session){
    ...
    output$warning_keyID_ui <- renderUI({
      ...
      if (input$choose_keyID =="AutoID") {
        ...
        dat <- tibble::as_tibble(Biobase::fData(rv$dataOut))[ ,input$choose_keyID]
    ...
    observe({
      ...
      if (input$choose_keyID =="AutoID") {
        ...
        test_keyID <- (length(tibble::as_tibble(Biobase::fData(rv$dataOut))[, input$choose_keyID])
                       == length(unique(tibble::as_tibble(Biobase::fData(rv$dataOut))[, input$choose_keyID])))
      }
